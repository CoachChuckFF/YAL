...
// --------- DEFINES -----------------------------------------
export const PEZ_PROGRAM_ID = new anchor.web3.PublicKey("GiJzLSGDvPMN7JGJsFxfuDoe8rnhAMEQGcMVEnDmgQbx");
...
export class PezProvider {
    ...
    static init = async (provider: anchor.Provider) => {
        return new PezProvider(
            provider,
            await PezProvider._getPezProgram(provider),
        );
    }

    static _getPezProgram = async (provider: anchor.Provider) => { 
        const idl = await anchor.Program.fetchIdl(PEZ_PROGRAM_ID, provider);
        return new anchor.Program<anchor.Idl>(idl as any, PEZ_PROGRAM_ID, provider);
    }
}
...
export const getPezDispenser = async (
    ...
    if((pezKey as PezDispenser).nonce){
        if( shouldUpdate ){
            return (await pezProvider.program.account.pezDispenser.fetch((pezKey as PezDispenser).dispenser)) as PezDispenser; 
        } else {
            return await pezKey as PezDispenser;
        }
    }
    return (await pezProvider.program.account.pezDispenser.fetch(pezKey as web3.PublicKey)) as PezDispenser; 
}
...
export const createPezDispenser = async (
    ...
    const pezParams = {
        ...
        candyPerWallet: new BN(3*1_000_000),
        candyPerPull: new BN(3),
    } as CreatePezParams;
    
    await pezProvider.program.rpc.createPez(
        pezParams,
        {
          accounts: {
...
export const updatePezDispenser = async (
    ...
    await pezProvider.program.rpc.updatePez(
      pezParams,
      {
        accounts: {
...
export const loadPezDispenser = async (
    ...
    await pezProvider.program.rpc.loadPez(
      pezParams,
      {
        accounts: {
          dispenser: pez.dispenser,
...
export const emptyPezDispenser = async (
    ...
    await pezProvider.program.rpc.emptyPez(
      pezParams,
      {
        accounts: {
          dispenser: pez.dispenser,
